...
static void transformJsonTableColumns(ParseState *pstate,
...
transformJsonTable(ParseState *pstate, JsonTable *jt)
{
	...
	tf->plan = (Node *) makeJsonTablePlan(rootPathSpec, jt->on_error);

	/* Transform jt->columns to add their JsonExpr nodes to tf->colvalexprs. */
	transformJsonTableColumns(pstate, jt, tf, exprType(tf->docexpr));
...
/*
 * Append transformed JsonTableColumn nodes, which become JsonExpr nodes,
 * to TableFunc.colvalexprs.
 */
static void
transformJsonTableColumns(ParseState *pstate, JsonTable *jt,
						  TableFunc *tf, Oid contextItemTypid)
{
