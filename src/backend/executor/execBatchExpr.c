...
/*
 * BatchQualExec
 *		Evaluate all clauses over a RowBatch using an index survivor array.
 *
 * survivors[] is caller-allocated with capacity b->nrows.  On entry it
 * is uninitialized; on return survivors[0..nqualified-1] holds the indices
 * (into the current batch) of tuples that passed all clauses.
 *
 * For each clause, the survivor set is compacted in place: each candidate
 * index is re-pointed via repoint_slot, deformed up to max_attno, and
 * tested.  Only passing indices are kept.
 *
			  ...
			  int *survivors)
{
	...
	/* Seed survivor set with all row indices */
	...
	for (int c = 0; c < bqs->nclauses; c++)
	{
		...
		{
			int		idx = survivors[i];
			TupleTableSlot *slot = RowBatchGetSlot(b, idx);
			bool	pass;

			slot_getsomeattrs(slot, bqs->max_attno);
