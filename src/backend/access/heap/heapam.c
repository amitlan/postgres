 ...
 * the scan's rs_tupledata[] array.  Unlike the previous version there is
 * no separate tuple header storage in HeapPageBatch itself; rs_tupledata[]
 ...
 *
 * A single TupleTableSlot is created with TTSOpsHeapTuple ops.  The batch
 * path re-points this slot to each tuple in turn rather than allocating N
 * slots.  TTSOpsHeapTuple rather than TTSOpsBufferHeapTuple is used because
 * HeapPageBatch holds a single buffer pin for the entire page; per-slot pin
 ...
 *
 * Serves slices from the current page's rs_tupledata[] array.  If the
 ...
 *
 * hb->tuples points directly into scan->rs_tupledata[]; the entries remain
...
heap_getnextbatch(TableScanDesc sscan, RowBatch *b, ScanDirection dir)
{
	...
	if (remaining > 0)
	{
		...
		hb->servefrom = &hb->tuples[hb->nextitem];
		hb->ntuples = nserve;
		hb->nextitem += nserve;
	...
	for (;;)
	{
		 ...
		 * scan->rs_tupledata[0..rs_ntuples-1] via page_collect_tuples().
		 */
		...
		if (scan->rs_ntuples > 0)
		{
			...
			{
				HeapScanVisItem *vitem = &scan->rs_vistuples[i];
				HeapTuple tup = &hb->tuples[i];

				tup->t_data = (HeapTupleHeader) ((char *) page + vitem->t_off);
				tup->t_len = vitem->t_len;
				ItemPointerSet(&tup->t_self, scan->rs_cblock, vitem->t_offnum);
				tup->t_tableOid = sscan->rs_rd->rd_id;
			}
			...
			hb->servefrom = &hb->tuples[0];
			hb->ntuples = scan->rs_ntuples;
			hb->nextitem = nserve;
 ...
 * heap_repoint_slot
 *
 * Re-point the batch's single slot to hb->tuples[idx].  When the buffer
 * has not changed since the last call -- the common case within a batch --
 * only the fields that vary between tuples are updated, bypassing
 * ExecStoreBufferHeapTuple.  When the buffer changes (first tuple of a new
 * page), ExecStoreBufferHeapTuple is called to acquire a pin on the new
 * buffer, release the old one, and reset slot metadata correctly.
 *
 * hb->tuples[idx] was populated by page_collect_tuples() via
...
{
	HeapPageBatch			  *hb = (HeapPageBatch *) b->am_payload;
	BufferHeapTupleTableSlot  *bslot = (BufferHeapTupleTableSlot *) b->slot;
	HeapTuple				   tup = &hb->servefrom[idx];

	Assert(idx >= 0 && idx < hb->ntuples);
	Assert(TTS_IS_BUFFERTUPLE(b->slot));

	if (unlikely(bslot->buffer != hb->buf))
	{
		ExecStoreBufferHeapTuple(tup, b->slot, hb->buf);
		return;
	}

	/*
	 * TTS_FLAG_EMPTY was cleared by ExecStoreBufferHeapTuple on the first
	 * repoint of this page and is not set again within a batch.
	 */
	bslot->base.tuple = tup;
	bslot->base.off = 0;
	bslot->base.base.tts_nvalid	= 0;
	bslot->base.base.tts_tid = tup->t_self;
}
