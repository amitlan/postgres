			   ...
			   uint32 flags)
{
	...
	scan->rs_tupledata = palloc(sizeof(HeapTupleData) * MaxHeapTuplesPerPage);
...
heap_endscan(TableScanDesc sscan)
{
	...
	pfree(scan->rs_tupledata);
	scan->rs_tupledata = NULL;
	pfree(scan);
}
...
heap_getnextbatch(TableScanDesc sscan, RowBatch *b, ScanDirection dir)
{
	...
	if (remaining > 0)
	{
		...
		hb->tuples = &scan->rs_tupledata[hb->nextitem];
		hb->ntuples = nserve;
		hb->nextitem += nserve;
