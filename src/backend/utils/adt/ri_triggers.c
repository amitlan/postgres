...
ri_FastPathEndBatch(void *arg)
{
	...
	/*
	 * Release this cycle's entries and remove them from the cache; leave outer
	 * cycles' entries for their own callbacks.  Destroy the cache once empty.
	 */
	/* Release and remove this cycle's entries; drop the cache once empty. */
			   ...
			   SubTransactionId parentSubid)
{
	 ...
	 * still cleaned up, rather than stranding it under a vanished subxact id.
	 *
	 * We touch no relations, locks or buffers -- the ResourceOwner handled
	 * those.  The entry's slots and flush context are memory in
	 * TopTransactionContext, freed at end-of-transaction reset; we only remove
	 * the hash entry so it is not reused or torn down again.
	 */
	hash_seq_init(&status, ri_fastpath_cache);
	while ((entry = hash_seq_search(&status)) != NULL)
	{
...
ri_FastPathGetEntry(const RI_ConstraintInfo *riinfo, Relation fk_rel)
{
	...
	if (!found)
	{
		 ...
		 * RegisterAfterTriggerBatchCallback() appends to the current after-trigger
		 * query depth's callback list and fires at that depth's
		 * AfterTriggerEndQuery(), so a nested cycle needs its own registration; a
		 * single global latch would leave the nested cycle's list empty and its
		 * batch unflushed.  Pass the depth as the callback arg so the callback
		 * flushes only its own cycle's entries.
		 */
		{
			...
			/*
			 * Register the callback once per firing cycle (query depth), including
			 * the deferred cycle at depth -1.  Rather than track registered depths
			 * separately, check whether any other cache entry was already created
			 * at this depth: if so, its creation already registered the callback
			 * for this cycle.  (The just-created entry is already in the hash, so
			 * skip it by conoid.)
			 */
