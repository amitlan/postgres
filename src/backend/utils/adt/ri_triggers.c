					  ...
					  Snapshot snapshot, IndexScanDesc scandesc)
{
	...
	while (index_getnext_slot(scandesc, ForwardScanDirection, pk_slot))
	{
		 ...
		 *
		 * A foreign key may reference a nullable unique column, not just a NOT
		 * NULL primary key.  If ri_LockPKTuple() chased an update chain to a
		 * version whose referenced key is now NULL, that version cannot equal
		 * any buffered (non-null) FK value, so skip it.  This mirrors the SPI
		 * path, where the requalifying "pkatt = $n" yields NULL and the row is
		 * not returned.
		 */
						 ...
						 TupleTableSlot *new_slot)
{
	...
	for (int i = 0; i < nkeys; i++)
	{
		...
		/*
		 * A foreign key may reference a nullable unique column, so the version
		 * we chased the update chain to may have a NULL in a key column.  A
		 * NULL never equals the value we searched for, so treat it as no
