...
--
-- Pruning-aware locking for prepared DML.  A reused generic plan locks
-- only the partitions it needs instead of all of them, but for a
-- partitioned ModifyTable the "needed" set is the pruning survivor(s)
-- PLUS the ModifyTable's first result relation, which is always locked:
-- ExecInitModifyTable() requires a usable result relation as a template
-- even when initial pruning removes every target partition (see
-- PlannedStmt.firstResultRels / setrefs.c).  So the locked leaves are
-- {first result relation} + {survivors}, which collapses to just the
-- survivor when the survivor happens to be the first partition.
--
-- The first result relation here is prunelock_p1 (lowest-numbered leaf
-- in partition-bound order).  All three single-statement DML strategies
-- are eligible (CachedPlanCanPrep, list_length(stmt_list) == 1):
--   * PORTAL_MULTI_QUERY   -- plain UPDATE/DELETE, no RETURNING
--   * PORTAL_ONE_RETURNING -- UPDATE/DELETE ... RETURNING
--   * PORTAL_ONE_MOD_WITH  -- SELECT with a data-modifying CTE
-- (The multi-statement fallback, which locks *all* partitions, is
-- covered separately by the rule-rewrite test below.)
--
insert into prunelock_p select generate_series(1, 3);

-- PORTAL_MULTI_QUERY: survivor == first result relation, so just p1.
prepare prunelock_upd (int) as update prunelock_p set a = a where a = $1;
explain (costs off) execute prunelock_upd(1);      -- build generic plan
begin;
execute prunelock_upd(1);                          -- reuse: prune-before-lock
select c.relname
  from pg_locks l join pg_class c on c.oid = l.relation
 where l.pid = pg_backend_pid()
   and c.relname like 'prunelock_p_'
 order by c.relname;                               -- expect: p1
commit;
deallocate prunelock_upd;

-- PORTAL_ONE_RETURNING: survivor p2 + first result relation p1.
prepare prunelock_ret (int) as
  update prunelock_p set a = a where a = $1 returning a;
explain (costs off) execute prunelock_ret(2);
begin;
execute prunelock_ret(2);
select c.relname
  from pg_locks l join pg_class c on c.oid = l.relation
 where l.pid = pg_backend_pid()
   and c.relname like 'prunelock_p_'
 order by c.relname;                               -- expect: p1, p2
commit;
deallocate prunelock_ret;

-- PORTAL_ONE_MOD_WITH: survivor p3 + first result relation p1.
prepare prunelock_mw (int) as
  with m as (update prunelock_p set a = a where a = $1 returning a)
  select count(*) from m;
explain (costs off) execute prunelock_mw(3);
begin;
execute prunelock_mw(3);
select c.relname
  from pg_locks l join pg_class c on c.oid = l.relation
 where l.pid = pg_backend_pid()
   and c.relname like 'prunelock_p_'
 order by c.relname;                               -- expect: p1, p3
commit;
deallocate prunelock_mw;
